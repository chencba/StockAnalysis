...
class StockPredictor:
    ...
    def convert_date(self, year, month, day):
        """Convert the year, month and day to datetime."""
        return dt.datetime(year, month, day)

    def get_predict_date(self):
        """Get the year, month and day for prediction date."""
        predict_date = None
        while predict_date is None:
            try:
                year = int(input("Please input the year you would like to predict: "))
                month = int(input("Please input the month you would like to predict: "))
                day = int(input("Please input the day you would like to predict: "))
                try:
                    predict_date = self.convert_date(year, month, day)
    ...
    def main(self):
        ...
        y, m, d = self.get_predict_date()
        pred_date = np.array([y, m, d]).reshape(1, -1) # prediction date
        # Linear Regression Model
        ...
        pdate = self.convert_date(y, m, d)
        fsteps = self.to_integer(pdate) - self.to_integer(last_train_date)  # Number of future steps
        ...
        mse3, r23 = self.evaluate(test, y_pred3) # evaluation scores for model3
        print("MSE: ", mse3)
